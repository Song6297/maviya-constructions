...
const FundManagement = {
    ...
    async getProjectBalance(projectId) {
        ...
        const virtualBalance = parseFloat(wallet.virtualBalance) || 0;
        const advanceReceived = parseFloat(wallet.advanceReceived) || 0;
        const pendingDues = parseFloat(wallet.pendingDues) || 0;
        const totalLoansGiven = parseFloat(wallet.totalLoansGiven) || 0;
        const totalLoansReceived = parseFloat(wallet.totalLoansReceived) || 0;
        ...
        return {
            ...
            totalLoansGiven,
            totalLoansReceived,
            netBalance: virtualBalance - totalLoansReceived + totalLoansGiven
        };
    },
    ...
    async getProjectFinancialSummary(projectId) {
        ...
        const [balance, loansGiven, loansReceived, paymentAllocations] = await Promise.all([
            this.getProjectBalance(projectId),
        ...
        // Calculate active loans (handle null/undefined arrays)
        ...
        const totalActiveLoansGiven = activeLoansGiven.reduce((sum, l) => sum + (parseFloat(l.amount) - parseFloat(l.settlementAmount || 0)), 0);
        const totalActiveLoansReceived = activeLoansReceived.reduce((sum, l) => sum + (parseFloat(l.amount) - parseFloat(l.settlementAmount || 0)), 0);
        
        const virtualBalance = parseFloat(balance.virtualBalance) || 0;
        const totalPayments = (paymentAllocations || []).reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);

        return {
            projectId,
            virtualBalance: virtualBalance,
            advanceReceived: parseFloat(balance.advanceReceived) || 0,
            pendingDues: parseFloat(balance.pendingDues) || 0,
            activeLoansGiven: totalActiveLoansGiven,
            activeLoansReceived: totalActiveLoansReceived,
            netAvailableBalance: virtualBalance - totalActiveLoansReceived,
    ...
    async getOverallFundStatus() {
        const [allWallets, allProjects] = await Promise.all([
            Storage.projectWallets.getAll(),
            Storage.projects.getAll()
        ]);
        
        // Fetch all project summaries in parallel for faster loading
        ...
        return {
            ...
            netBankBalance: totalVirtualBalance,
            projectSummaries,
