...
const WorkerManagement = {
    ...
    async renderLabour(projectId) {
        ...
        if (!container) {
            console.warn('labourCardsContainer not found');
            return;
        }
        ...
        if (!assignments.length) {
            ...
            if (attendanceSummary) attendanceSummary.classList.add('hidden');
            return;
        }

        if (empty) empty.classList.add('hidden');
        if (totalRow) totalRow.classList.remove('hidden');
        if (attendanceSummary) attendanceSummary.classList.remove('hidden');

        // Get today's date
        const today = new Date().toISOString().split('T')[0];
        const attendanceDateEl = document.getElementById('attendanceDate');
        if (attendanceDateEl) {
            attendanceDateEl.textContent = new Date().toLocaleDateString('en-IN', { 
        ...
        const labourTotalAmountEl = document.getElementById('labourTotalAmount');
        const labourTotalPaidEl = document.getElementById('labourTotalPaid');
        const labourTotalBalanceEl = document.getElementById('labourTotalBalance');
        
        if (labourTotalAmountEl) labourTotalAmountEl.textContent = `₹${totalEarned.toLocaleString('en-IN')}`;
        if (labourTotalPaidEl) labourTotalPaidEl.textContent = `₹${totalPaid.toLocaleString('en-IN')}`;
        if (labourTotalBalanceEl) labourTotalBalanceEl.textContent = `₹${Math.max(0, totalBalance).toLocaleString('en-IN')}`;

        // Update attendance summary with null checks
        const totalWorkersEl = document.getElementById('totalWorkers');
        const presentWorkersEl = document.getElementById('presentWorkers');
        const absentWorkersEl = document.getElementById('absentWorkers');
        const halfDayWorkersEl = document.getElementById('halfDayWorkers');
        
        if (totalWorkersEl) totalWorkersEl.textContent = assignments.length;
        if (presentWorkersEl) presentWorkersEl.textContent = presentCount;
        if (absentWorkersEl) absentWorkersEl.textContent = absentCount;
        if (halfDayWorkersEl) halfDayWorkersEl.textContent = halfDayCount;
    },
    ...
    async loadAttendanceForDate(date, projectId) {
        ...
        if (!attendanceList) {
            console.warn('attendanceList element not found');
            return;
        }
    ...
    openPaymentModal(workerId, assignmentId, workerName, pendingAmount) {
        const workerIdInput = document.getElementById('paymentWorkerId');
        const assignmentIdInput = document.getElementById('paymentAssignmentId');
        const workerNameEl = document.getElementById('paymentWorkerName');
        const pendingAmountEl = document.getElementById('paymentPendingAmount');
        const amountInput = document.getElementById('paymentAmount');
        const dateInput = document.getElementById('paymentDate');
        const modal = document.getElementById('workerPaymentModal');
        
        if (!modal) {
            console.warn('workerPaymentModal not found');
            return;
        }
        ...
        if (pendingAmountEl) pendingAmountEl.textContent = `₹${pendingAmount.toLocaleString('en-IN')}`;
