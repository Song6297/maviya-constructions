...
const ProjectApp = {
    ...
    async renderFundStatus() {
        try {
            if (!window.FundManagement) {
                ...
                document.getElementById('virtualBalance').textContent = '₹0';
                document.getElementById('loansGiven').textContent = '₹0';
                document.getElementById('loansReceived').textContent = '₹0';
                document.getElementById('netAvailable').textContent = '₹0';
                return;
            }

            // Get project financial summary
            const fundSummary = await FundManagement.getProjectFinancialSummary(this.projectId);
            console.log('Fund Summary:', fundSummary);
            
            // Ensure values are numbers
            const virtualBalance = parseFloat(fundSummary.virtualBalance) || 0;
            const loansGiven = parseFloat(fundSummary.activeLoansGiven) || 0;
            const loansReceived = parseFloat(fundSummary.activeLoansReceived) || 0;
            const netAvailable = parseFloat(fundSummary.netAvailableBalance) || 0;
            
            // Update virtual wallet metrics
            document.getElementById('virtualBalance').textContent = `₹${Utils.formatNumber(virtualBalance)}`;
            document.getElementById('loansGiven').textContent = `₹${Utils.formatNumber(loansGiven)}`;
            document.getElementById('loansReceived').textContent = `₹${Utils.formatNumber(loansReceived)}`;
            document.getElementById('netAvailable').textContent = `₹${Utils.formatNumber(netAvailable)}`;
    ...
    async generatePDF() {
        ...
        try {
            ...
            // Theme Colors (Playful Construction)
            const colors = {
                primary: [91, 155, 213],      // Blueprint Blue
                secondary: [255, 140, 97],    // Brick Orange
                success: [127, 216, 190],     // Mint Green
                warning: [255, 184, 77],      // Safety Yellow
                purple: [186, 104, 200],      // Violet
                pink: [236, 72, 153],         // Pink
                teal: [20, 184, 166],         // Teal
                dark: [44, 62, 80],           // Slate
                light: [232, 244, 248],       // Light blue
                white: [255, 255, 255]
            };
            ...
            doc.setTextColor(...colors.primary);
            doc.setFont('helvetica', 'bold');
            doc.text('Maviya Constructions', 50, y);
            y += 7;
            ...
            // Project Name Box with rounded corners effect
            doc.setFillColor(...colors.secondary);
